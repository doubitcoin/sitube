...
// Copyright (c) 2018 The Goro developers
...
|   _\  /   \  |  |  /   \
 ...
 ____/  \___/  |   \ \__ /
    ...
    if (testnet_mode) {
      logger(INFO) << "Starting goro in testnet mode!";
    }
    ...
    if (!coreConfig.configFolderDefaulted) {
      if (!Tools::directoryExists(coreConfig.configFolder)) {
        throw std::runtime_error("Goro Directory does not exist: " + coreConfig.configFolder);
      }
    } else {
      if (!Tools::create_directories_if_necessary(coreConfig.configFolder)) {
        throw std::runtime_error("Can't create goro  directory: " + coreConfig.configFolder);
      }
    }
    ...
    logger(INFO) << "Initializing goro p2p server...";
    if (!p2psrv.init(netNodeConfig)) {
      logger(ERROR, BRIGHT_RED) << "Failed to initialize goro p2p server.";
      return 1;
    }
    logger(INFO) << "Goro P2p server initialized OK";

    logger(INFO) << "Initializing goro...";
    if (!ccore.init(coreConfig, minerConfig, true)) {
      logger(ERROR, BRIGHT_RED) << "Failed to initialize goro  core";
      return 1;
    }
    logger(INFO) << "Goro initialized OK";
    ...
    logger(INFO) << "Starting goro  rpc server on address " << rpcConfig.getBindAddress();
    ...
    logger(INFO) << "Goro rpc server started ok";
    ...
    logger(INFO) << "Starting goro p2p net loop...";
    p2psrv.run();
    logger(INFO) << "Goro p2p net loop stopped";

    dch.stop_handling();

    logger(INFO) << "Stopping goro rpc server...";
    rpcServer.stop();

    logger(INFO) << "Deinitializing goro...";
    ccore.deinit();
